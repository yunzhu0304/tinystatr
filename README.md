...
In addition to automatic method selection, *tinystatr* also allows users to manually force parametric or non-parametric procedures through the `parametric` argument.
...
🎛️ Manual control with `parametric`

Both `stat2()` and `stat3()` support the `parametric` argument:

- `parametric = NULL`
    
    Automatic method selection based on data characteristics. This is the default behavior.
    
- `parametric = TRUE`
    
    Force a **parametric procedure**.
    
    - In `stat2()`: choose between **t-test** and **Welch’s t-test** according to variance homogeneity.
    
    - In `stat3()`: force **ANOVA** followed by post-hoc testing.
    
- `parametric = FALSE`
    
    Force a **non-parametric procedure**.
    
    - In `stat2()`: force the **Wilcoxon test**.
    
    - In `stat3()`: force the **Kruskal-Wallis test** followed by **Dunn’s test**.

<img width="409" alt="Snipaste_2024-04-27_22-29-21" src="https://github.com/yunzhu0304/tinystatr/assets/146269920/c21a7aa5-62ad-4ffa-9713-073d507fb427">
